-- ============================================================
-- NXTUP — Fix: pay corre DESPUÉS del UPDATE de barbers
-- Run in Supabase SQL Editor
--
-- Bug detectado en operación:
--
--   El existente Jesus toca busy → active esperando quedar arriba
--   de Pascual (gracias al bump de available_since que hace
--   pay_late_arrival_toll en la 022). Pero pasa DEBAJO de Pascual.
--
-- Causa: orden de operaciones incorrecto en device_update_barber_
-- state. El flujo era:
--
--   1. Cleanup queue_entries
--   2. perform pay_late_arrival_toll(...)  ← bumpea available_since
--   3. ... break invalidation, held position calc ...
--   4. UPDATE barbers SET available_since = v_next_avail
--      ← SOBRESCRIBE el bump!
--
-- El UPDATE del paso 4 mataba el bump del paso 2. Resultado:
-- available_since queda en now(), que es DESPUÉS del available_
-- since del tarde → existente queda debajo del tarde.
--
-- Fix: mover el pay para que corra DESPUÉS del UPDATE del barber.
-- Mismo fix en el state route TS en el commit acompañante.
-- ============================================================
...
begin
  ...
  if p_target = 'available' then
    -- Cleanup queue_entries (in_progress + called si veníamos de busy).
    ...
    -- ── PAY MOVIDO A AQUÍ (post-UPDATE) — fix del 027 ───────
    -- Antes corría ANTES del UPDATE de arriba. El bump de
    -- available_since que hace pay_late_arrival_toll (migration
    -- 022) quedaba sobrescrito por el UPDATE. Ahora corre
    -- después: el UPDATE setea v_next_avail (now()), el pay
    -- baja available_since al del tarde - 1s si todavía debe
    -- cortes. LEAST() en pay() asegura que no movemos hacia
    -- adelante por accidente.
    if v_from_status = 'busy' then
