...
// Faster button repaint for the optimistic-UI path: only redraw the
// TWO buttons whose appearance actually changed (the previously
// selected one becomes outlined, the newly selected one becomes
// filled). The third button doesn't change, so skipping it saves the
// ~15ms cost of clearing and redrawing it. Total touch→visual budget
// for the button strip drops from ~50ms (3 buttons) to ~30ms (2).
static void drawButtonByIdx(int idx, bool selected) {
  switch (idx) {
    case 0: drawButton(0, "ACTIVE", nxtup::kActive, selected); break;
    case 1: drawButton(1, "BUSY",   nxtup::kBusy,   selected); break;
    case 2: drawButton(2, "BREAK",  nxtup::kBreak,  selected); break;
  }
}

static int stateToBtnIdx(BarberState s) {
  switch (s) {
    case ST_ACTIVE: return 0;
    case ST_BUSY:   return 1;
    case ST_BREAK:  return 2;
  }
  return -1;
}

static void drawTwoChangedButtons(int oldIdx, int newIdx) {
  // Repaint old (was filled, now outline) first, then new (was
  // outline, now filled). Order matters only if they overlapped,
  // which they don't here.
  if (oldIdx >= 0 && oldIdx != newIdx) {
    drawButtonByIdx(oldIdx, false);
  }
  drawButtonByIdx(newIdx, true);
}
...
static void onButtonTap(int idx) {
  ...
  //
  // Snapshot the OLD button index BEFORE changing g_state so the
  // partial repaint below knows which two buttons need updating.
  const int oldBtnIdx = stateToBtnIdx(g_state);
  ...
  // Order matters: the buttons are at the BOTTOM of the screen and
  // are what the barber's finger is on, so flipping them first gives
  // the most direct "I felt the tap" signal. The info panel above
  // (which costs a 800×250 fillRect + text) repaints right after.
  // Net perceived latency for the button highlight drops from ~100ms
  // to ~15-20ms.
  drawTwoChangedButtons(oldBtnIdx, idx);
  drawInfo();
